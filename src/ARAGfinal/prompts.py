...
# ╔═══════════════════════════════════════════════════════════════════════╗
# ║  NEW FUNCTION: GCN-enhanced UUA prompt (Strategy 1)                  ║
# ║                                                                      ║
# ║  This uses the 'user_understanding_gcn' template from prompts.yaml   ║
# ║  which already existed in your YAML but was unused in this flow.     ║
# ╚═══════════════════════════════════════════════════════════════════════╝
def create_summary_user_behavior_with_gcn_prompt(
    lt_ctx: str, cur_ses: str, gcn_behavior_insight: str
) -> str:
    template = _manager.get_template('user_understanding_gcn')
    return template.format(
        long_term_context=lt_ctx,
        current_session=cur_ses,
        gcn_behavior_insight=gcn_behavior_insight
    )
